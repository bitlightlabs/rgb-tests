...
mod integration_tests {
    ...
    fn test_sandbox_directory_isolation() {
        ...
        let base_files = [("base_file.txt", "base data"), ("shared_name.txt", "base version")];
        let delta_files = [("delta_file.txt", "delta data"), ("shared_name.txt", "delta version")];
        ...
        for (filename, content) in delta_files.iter() {
            std::fs::write(config.delta_path.join(filename), content)
                .expect(&format!("Should be able to write {} to delta dir", filename));
        }
        
        // Verify file existence and content isolation
        assert!(config.base_path.join("base_file.txt").exists(), "base_file.txt should exist in base");
        assert!(config.delta_path.join("delta_file.txt").exists(), "delta_file.txt should exist in delta");
        
        // Verify files don't cross-contaminate
        assert!(!config.base_path.join("delta_file.txt").exists(), "delta_file.txt should not exist in base");
        assert!(!config.delta_path.join("base_file.txt").exists(), "base_file.txt should not exist in delta");
        ...
        let delta_shared_content = std::fs::read_to_string(config.delta_path.join("shared_name.txt"))
            .expect("Should be able to read shared_name.txt from delta");
            
        assert_eq!(base_shared_content, "base version", "Base version of shared file should have correct content");
        assert_eq!(delta_shared_content, "delta version", "Delta version of shared file should have correct content");
        assert_ne!(base_shared_content, delta_shared_content, "Same filename should have different content in isolated directories");
    }
    ...
    fn test_sandbox_config_validation() {
        ...
        assert_eq!(config_same.base_path, config_same.delta_path, "Same path config should have identical paths");
        assert!(config_same.base_path.exists(), "Same path should exist");
        assert!(config_same.base_path.is_dir(), "Same path should be a directory");
        
        // Test 2: Different paths (typical scenario)
        let config_different = SandboxConfig::temp().expect("Failed to create config");
        assert_ne!(config_different.base_path, config_different.delta_path, "Different path config should have different paths");
        assert!(config_different.base_path.exists(), "Base path should exist in different config");
        assert!(config_different.delta_path.exists(), "Delta path should exist in different config");
        ...
        assert!(config_manual.base_path.is_absolute(), "Manual base path should be absolute");
        assert!(config_manual.delta_path.is_absolute(), "Manual delta path should be absolute");
        assert_ne!(config_manual.base_path, config_manual.delta_path, "Manual paths should be different");
        
        // Test 4: Path canonicalization behavior
        let canonical_base = config_manual.base_path.canonicalize().expect("Base path should be canonicalizable");
        let canonical_delta = config_manual.delta_path.canonicalize().expect("Delta path should be canonicalizable");
        
        assert_eq!(canonical_base, base_temp.path().canonicalize().unwrap(), "Base path should canonicalize correctly");
        assert_eq!(canonical_delta, delta_temp.path().canonicalize().unwrap(), "Delta path should canonicalize correctly");
    }

    #[test] 
    fn test_error_handling_coverage() {
        use poc::{SandboxError, SandboxConfig, SandboxStock};
        use std::fs;
        use std::path::PathBuf;
        
        // Test 1: Invalid base path handling
        let invalid_path = PathBuf::from("/nonexistent/invalid/path/that/should/not/exist");
        let config = SandboxConfig::new(invalid_path, tempfile::tempdir().unwrap().path().to_path_buf());
        
        // This should fail when trying to load from non-existent base
        let result = SandboxStock::load(config);
        assert!(result.is_err(), "Loading from invalid base path should fail");
        ...
        #[cfg(unix)]
        {
            ...
            let articles = poc::rgb_components::create_rgb20_articles().expect("Failed to create test articles");
            let state = poc::rgb_components::create_effective_state(&articles).expect("Failed to create test state");
            
            // First create a base StockFs, then try to create sandbox (which should fail due to delta permissions)
            let base = poc::rgb_components::create_test_stock(articles.clone(), state.clone(), base_temp.path().to_path_buf())
                .expect("Should be able to create base stock");
            let result = SandboxStock::create_with_base(config, base);
            
            // This should fail due to permissions
            assert!(result.is_err(), "Creating sandbox with read-only delta should fail");
        ...
        assert_eq!(format!("{}", error), "Data not found in either base or delta storage", "DataNotFound error should have correct display");
    ...
    fn test_articles_construction() {
        ...
        assert!(!articles.genesis().version.to_string().is_empty(), "Articles should have a valid version");
        
        // Verify the articles contain expected components
        let default_api = articles.default_api();
        assert!(!default_api.verifiers.is_empty(), "Articles should have API verifiers");
        ...
        assert!(!operation.contract_id.to_string().is_empty(), "ContractId should have a non-empty string representation");
        
        // Verify the CallId comes from Articles API
        let expected_call_id = default_api.verifiers.first_key_value()
            .map(|(_, call_id)| *call_id)
            .expect("Articles should have at least one API verifier");
        assert_eq!(operation.call_id, expected_call_id, "Operation CallId should match Articles API verifier");
    }
    ...
    fn test_complete_rgb_integration() {
        ...
        assert!(!articles.genesis().version.to_string().is_empty(), "Articles should have valid version");
        let api_verifier_count = articles.default_api().verifiers.len();
        assert!(api_verifier_count > 0, "Articles should have at least one API verifier");
        ...
        let expected_call_id = articles.default_api().verifiers.first_key_value()
            .map(|(_, call_id)| *call_id)
            .expect("Articles should have API verifiers");
        assert_eq!(operation.call_id, expected_call_id, "Operation CallId should match Articles API");

        // Step 4: Create StockFs
        let temp_dir = tempfile::tempdir().expect("Failed to create temp directory");
        let stock = rgb_components::create_test_stock(articles.clone(), effective_state.clone(), temp_dir.path().to_path_buf())
            .expect("Should be able to create StockFs with RGB components");
        
        // Verify StockFs creation and content
        assert_eq!(stock.articles().genesis().version, articles.genesis().version, "StockFs should preserve Articles version");
        // Note: RawState doesn't implement PartialEq, so we verify individual fields
        assert_eq!(stock.state().raw.auth.len(), effective_state.raw.auth.len(), "StockFs should preserve auth state length");
        assert_eq!(stock.state().raw.global.len(), effective_state.raw.global.len(), "StockFs should preserve global state length");  
        assert_eq!(stock.state().raw.owned.len(), effective_state.raw.owned.len(), "StockFs should preserve owned state length");
        assert_eq!(stock.operation_count(), 0, "New StockFs should start with 0 operations");
        ...
        assert!(!stock.has_operation(test_opid), "New StockFs should not have test operation");
    ...
    fn test_sandbox_stock_delta_over_base() {
        ...
        let articles = rgb_components::create_rgb20_articles()
            .expect("Should be able to create Articles");
        let effective_state = rgb_components::create_effective_state(&articles)
            .expect("Should be able to create EffectiveState");

        // Create sandbox configuration
        let config = SandboxConfig::temp()
            .expect("Should be able to create sandbox config");
        ...
        let base = poc::rgb_components::create_test_stock(articles.clone(), effective_state, base_temp.path().to_path_buf())
            .expect("Should be able to create base stock");
        let mut sandbox = SandboxStock::create_with_base(config.clone(), base)
            .expect("Should be able to create SandboxStock");
            
        // Verify initial state
        assert_eq!(sandbox.operation_count(), 0, "New sandbox should start with 0 operations");
        assert_ne!(config.base_path, config.delta_path, "Sandbox should use separate base and delta paths");
        ...
        assert!(!sandbox.has_operation(test_opid), "New sandbox should not have test operation");
        
        // Add operation to sandbox
        sandbox.add_operation(test_opid, &operation);
        
        // Verify operation was added
        assert!(sandbox.has_operation(test_opid), "Operation should be present after adding");
        assert!(sandbox.operation_count() > 0, "Operation count should increase after adding operation");
        
        // Verify operation data integrity
        let retrieved_op = sandbox.operation(test_opid);
        assert_eq!(retrieved_op.contract_id, operation.contract_id, "Retrieved operation should match original");
        assert_eq!(retrieved_op.call_id, operation.call_id, "Retrieved operation CallId should match original");
        
        // Test commit transaction
        sandbox.commit_transaction(); // Should not panic or error
        
        // Verify operation is still present after commit
        assert!(sandbox.has_operation(test_opid), "Operation should still be present after commit");
        ...
        for (iter_opid, iter_op) in sandbox.operations() {
            if iter_opid == test_opid {
                assert_eq!(iter_op.contract_id, operation.contract_id, "Operation from iterator should match original");
                found_in_iterator = true;
                break;
            }
        }
        assert!(found_in_iterator, "Added operation should appear in operations iterator");
    }
    ...
    fn test_sandbox_rollback_logical_isolation() {
        ...
        // Create SandboxStock - first create base, then sandbox
        let base_temp = tempfile::tempdir().expect("Failed to create temp dir for base");
        let base = rgb_components::create_test_stock(articles.clone(), effective_state, base_temp.path().to_path_buf())
            .expect("Should be able to create base stock");
        ...
        assert_eq!(initial_count, 2, "Should have 2 operations after adding both");
        ...
        assert!(sandbox.has_operation(opid1), "Operation 1 should still be present after commit");
        assert!(sandbox.has_operation(opid2), "Operation 2 should still be present after commit");
    }
}
