...
fn test_lightning_network_operation_restrictions() {
    ...
    assert!(delta_stockpile.rollback().is_ok(), "Rollback should succeed");
    assert!(delta_stockpile.commit_to_base().is_ok(), "Commit should succeed");
}
...
fn test_base_layer_file_scanning() {
    ...
    let issuer_file = "test-schema.1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef.issuer";
    fs::write(base_temp.path().join(issuer_file), b"mock issuer").expect("Failed to write issuer");
    
    // Create a mock contract directory  
    let contract_dir = "test-contract.abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890.contract";
    let contract_path = base_temp.path().join(contract_dir);
    fs::create_dir_all(&contract_path).expect("Failed to create contract dir");
    fs::write(contract_path.join("mock.dat"), b"mock contract data").expect("Failed to write contract data");
    ...
    println!("Found {} issuers, {} contracts in base layer", issuer_count, contract_count);
...
fn test_delta_layer_transactional_behavior() {
    ...
    let initial_entries: Vec<_> = fs::read_dir(delta_temp.path())
        ...
        .collect();
    assert_eq!(initial_entries.len(), 0, "Delta directory should start empty");
    ...
    assert!(!state_file.exists(), "State file should be removed after rollback");
    ...
    assert!(!state_file2.exists(), "State file should be cleaned after commit");
    
    // Delta directory should be empty again
    let final_entries: Vec<_> = fs::read_dir(delta_temp.path())
        .expect("Failed to read delta dir")
        .collect();
    assert_eq!(final_entries.len(), 0, "Delta directory should be empty after commit");
}
...
fn test_multiple_instances_isolation() {
    ...
    stockpile1.rollback().expect("Failed to rollback stockpile 1");
    assert!(!delta1_file.exists());
    assert!(delta2_file.exists()); // stockpile2's delta should be unaffected
    
    stockpile2.rollback().expect("Failed to rollback stockpile 2");
    assert!(!delta2_file.exists());
}
