...
fn create_test_base_with_mock_data(
) -> Result<(TempDir, TempDir, SandboxConfig), Box<dyn std::error::Error>> {
    ...
    fs::write(
        base_dir.join(
            "test-issuer.rgb1qw508d6qejxtdg4y5r3zarvaryvhw82zdkmqq2yyfd9wdnlsllp8qvgapec.issuer",
        ),
        b"mock issuer data",
    )?;

    // Create mock base contract directory (simulating funding tx setup)
    let contract_dir = base_dir.join(
        "test-contract.rgb1qw508d6qejxtdg4y5r3zarvaryvhw82zdkmqq2yyfd9wdnlsllp8qvgapec.contract",
    );
...
fn test_delta_stockpile_creation_and_basic_properties() {
    let (base_temp, delta_temp, config) =
        create_test_base_with_mock_data().expect("Failed to create test directories");
    ...
    assert_eq!(
        delta_entries.len(),
        0,
        "Delta directory should be empty initially"
    );
}
...
fn test_rollback_functionality() {
    ...
    assert!(base_temp
        .path()
        .join("test-issuer.rgb1qw508d6qejxtdg4y5r3zarvaryvhw82zdkmqq2yyfd9wdnlsllp8qvgapec.issuer")
        .exists());
}
...
fn test_commit_to_base_functionality() {
    ...
    delta_stockpile
        .commit_to_base()
        .expect("Failed to commit to base");
    ...
    assert_eq!(
        delta_entries.len(),
        0,
        "Delta directory should be clean after commit"
    );
...
fn test_lightning_network_restrictions_import_issuer() {
    let (_base_temp, _delta_temp, config) =
        create_test_base_with_mock_data().expect("Failed to create test directories");
...
fn test_query_behavior_base_only() {
    ...
    assert_eq!(
        delta_stockpile.issuers_count(),
        0,
        "Should count only base issuers"
    );
    assert_eq!(
        delta_stockpile.contracts_count(),
        0,
        "Should count only base contracts"
    );
...
fn test_multiple_rollback_commit_cycles() {
    ...
    delta_stockpile
        .rollback()
        .expect("Failed to rollback cycle 1");
    assert!(!delta_temp.path().join("cycle1.dat").exists());
    ...
    delta_stockpile
        .commit_to_base()
        .expect("Failed to commit cycle 2");
    ...
    delta_stockpile
        .rollback()
        .expect("Failed to rollback cycle 3");
    assert!(!delta_temp.path().join("cycle3.dat").exists());
