...
mod commit_to_base_tests {
    ...
    fn test_commit_to_base_file_operations() {
        ...
        assert!(
            ...
            "Base should have RGB files"
        );
        assert!(
            has_rgb_files(&config.delta_path),
            "Delta should have RGB files"
        );

        // Verify file contents are different (base vs delta)
        for (base_file, delta_file) in base_files.iter().zip(delta_files.iter()) {
            let base_content = std::fs::read_to_string(base_file).expect("Should read base file");
            let delta_content =
                std::fs::read_to_string(delta_file).expect("Should read delta file");

            assert_ne!(
                base_content, delta_content,
                "Base and delta files should have different content before merge"
            );
            assert!(
                base_content.contains("base"),
                "Base file should contain 'base'"
            );
            assert!(
                delta_content.contains("delta"),
                "Delta file should contain 'delta'"
            );
        }
    }
    ...
    fn test_commit_to_base_file_merge_simulation() {
        ...
        // Create base files
        create_test_pile_files(&config.base_path, "base").expect("Should create base files");

        // Create delta files with different content
        create_test_pile_files(&config.delta_path, "delta").expect("Should create delta files");
        ...
        assert_eq!(
            initial_delta_count, 6,
            "Should have 6 delta files initially"
        );
        ...
        for entry in delta_entries {
            ...
            if delta_file.is_file() {
                ...
                std::fs::copy(&delta_file, &base_file).expect("Should copy delta file to base");
            }
        }
        ...
        assert_eq!(
            final_base_count, 6,
            "Should still have 6 base files after merge"
        );
        ...
        for entry in base_entries {
            ...
            let content =
                std::fs::read_to_string(&base_file).expect("Should read merged base file");
            assert!(
                content.contains("delta"),
                "Base file should now contain delta content after merge: {:?}",
                base_file
            );
        }
    }
    ...
    fn test_commit_to_base_delta_cleanup_expectation() {
        ...
        assert!(
            has_rgb_files(&config.delta_path),
            "Delta should have files before commit"
        );
        assert_eq!(
        ...
        for delta_file in delta_files {
            std::fs::remove_file(&delta_file).expect("Should be able to remove delta file");
        }

        // Verify cleanup
        assert!(
            !has_rgb_files(&config.delta_path),
            "Delta should be empty after cleanup"
        );
        assert_eq!(
            count_rgb_files(&config.delta_path),
            0,
            "Should have 0 delta files after cleanup"
        );

        // Verify delta directory still exists but is empty
        assert!(
            config.delta_path.exists(),
            "Delta directory should still exist"
        );
        assert!(
            config.delta_path.is_dir(),
            "Delta path should still be a directory"
        );
    }
    ...
    fn test_commit_to_base_idempotency() {
        ...
        let initial_base_count = count_rgb_files(&config.base_path);
        let initial_base_content: Vec<_> = base_files
            .iter()
        ...
        for _iteration in 0..3 {
            ...
            assert_eq!(
                current_base_count, initial_base_count,
            ...
            for (i, base_file) in base_files.iter().enumerate() {
                let current_content =
                    std::fs::read_to_string(base_file).expect("Should read base file");
                assert_eq!(
                    current_content, initial_base_content[i],
    ...
    fn test_commit_to_base_error_conditions() {
        ...
        assert!(
            !invalid_config.base_path.exists(),
            "Base should not exist for error test"
        );
        assert!(
            invalid_config.delta_path.exists(),
            "Delta should exist for error test"
        );
        ...
        assert!(
            invalid_config2.base_path.exists(),
            "Base should exist for error test"
        );
        assert!(
            !invalid_config2.delta_path.exists(),
            "Delta should not exist for error test"
        );
    }
