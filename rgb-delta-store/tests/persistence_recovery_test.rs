...
mod persistence_recovery_tests {
    ...
    fn create_persistent_test_data(
        dir: &Path,
        prefix: &str,
        session_id: u32,
    ...
    fn test_persistence_basic_file_survival() {
        ...
        assert!(
            ...
            "Base should have files after session 1"
        );
        assert_eq!(
            count_rgb_files(&config.base_path),
            6,
            "Should have 6 base files"
        );
        assert!(
            verify_session_data(&session_1_files, 1),
            "Files should contain session 1 data"
        );

        // Simulate program restart - files should still exist
        assert!(
            has_rgb_files(&config.base_path),
            "Base files should survive restart"
        );
        assert_eq!(
            count_rgb_files(&config.base_path),
            6,
            "File count should survive restart"
        );
        ...
        assert!(
            ...
            "Delta should have files after session 2"
        );
        assert_eq!(
            count_rgb_files(&config.delta_path),
            6,
            "Should have 6 delta files"
        );

        // Both base and delta should coexist
        assert!(
            has_rgb_files(&config.base_path),
            "Base should still exist with delta"
        );
        assert!(
            has_rgb_files(&config.delta_path),
            "Delta should exist with base"
        );
        assert!(
            verify_session_data(&session_1_files, 1),
            "Base data should be unchanged"
        );
        assert!(
            verify_session_data(&session_2_files, 2),
            "Delta data should be correct"
        );
    }
    ...
    fn test_persistence_state_recovery_after_restart() {
        ...
        let initial_file_sizes: Vec<_> = base_files
            .iter()
        ...
        for (i, file) in base_files.iter().enumerate() {
            ...
            assert_eq!(
                recovered_size, initial_file_sizes[i],
                "File size should be preserved during recovery: {:?}",
                file
            );
        }
        ...
        assert!(
            verify_session_data(&base_files, 1),
        ...
        assert!(
            verify_session_data(&base_files, 1),
            "Recovered base data should be intact"
        );
        assert!(
            verify_session_data(&delta_files, 2),
            "New delta data should be correct"
        );
    }
    ...
    fn test_persistence_multiple_restart_cycles() {
        ...
        for cycle in 1..=5 {
            ...
            assert!(
                has_rgb_files(&config.base_path),
                "Base should persist across cycle {}",
                cycle
            );
            ...
            // Verify cycle data
            assert!(
                verify_session_data(&cycle_files, cycle),
                "Cycle {} data should be correct",
                cycle
            );
            ...
            for cycle_file in cycle_files {
                ...
                std::fs::copy(&cycle_file, &base_file).expect("Should merge delta to base");

                // Clean up delta
                std::fs::remove_file(&cycle_file).expect("Should clean up delta file");
            }

            // Verify delta cleanup
            assert!(
                !has_rgb_files(&config.delta_path),
                "Delta should be cleaned up after cycle {}",
                cycle
            );
            ...
            assert!(
                verify_session_data(&base_files, cycle),
                "Base should contain cycle {} data after merge",
                cycle
            );
        }
    }
    ...
    fn test_persistence_partial_file_corruption_recovery() {
        ...
        std::fs::write(corrupt_file, "CORRUPTED DATA").expect("Should corrupt file");
        ...
        for file in intact_files {
            let content = std::fs::read_to_string(file).expect("Should read intact file");
            assert!(
                content.contains("session 1"),
                "Intact file should have correct content: {:?}",
                file
            );
        }
        ...
        assert!(
            verify_session_data(&base_files, 1),
            "Should recover after restoration"
        );
    }
    ...
    fn test_persistence_directory_structure_recovery() {
        ...
        assert!(
            has_rgb_files(&config.base_path),
            "Base should have root files"
        );
        assert!(
            has_rgb_files(&config.delta_path),
            "Delta should have root files"
        );

        // Simulate restart - verify structure persists
        assert!(
            base_subdir.exists(),
            "Base subdirectory should survive restart"
        );
        assert!(
            delta_subdir.exists(),
            "Delta subdirectory should survive restart"
        );
        assert!(
            base_nested_file.exists(),
            "Base nested file should survive restart"
        );
        assert!(
            delta_nested_file.exists(),
            "Delta nested file should survive restart"
        );

        // Verify content integrity
        let base_content =
            std::fs::read_to_string(&base_nested_file).expect("Should read base nested file");
        let delta_content =
            std::fs::read_to_string(&delta_nested_file).expect("Should read delta nested file");

        assert_eq!(
            base_content, "nested base data",
            "Base nested content should be intact"
        );
        assert_eq!(
            delta_content, "nested delta data",
            "Delta nested content should be intact"
        );
    }
    ...
    fn test_persistence_concurrent_write_safety() {
        ...
        for (i, file) in concurrent_files.iter().enumerate() {
            let content = std::fs::read_to_string(file).expect("Should read concurrent file");
            assert_eq!(
                content,
                format!("concurrent write {}", i),
                "Concurrent write {} should be intact",
                i
            );
        }
        ...
        assert!(
            has_rgb_files(&config.base_path),
            "Original base data should be intact"
        );
    ...
    fn test_persistence_large_file_handling() {
        ...
        for file in large_files.iter() {
            std::fs::write(file, &large_content).expect("Should write large file");
        }

        // Verify large files were written correctly
        for file in large_files.iter() {
            let content = std::fs::read_to_string(file).expect("Should read large file");
            assert_eq!(content.len(), 10000, "Large file should have correct size");
            assert_eq!(
                content, large_content,
                "Large file content should be intact"
            );
        }
        ...
        for file in large_files.iter() {
            assert!(
                file.exists(),
                "Large file should survive restart: {:?}",
                file
            );

            let recovered_content =
                std::fs::read_to_string(file).expect("Should read large file after restart");
            assert_eq!(
            ...
            assert_eq!(
                recovered_content, large_content,
    ...
    fn test_persistence_empty_state_recovery() {
        ...
        assert!(
            !has_rgb_files(&config.delta_path),
            "Delta should start empty"
        );

        // Simulate restart with empty state
        assert!(
            !has_rgb_files(&config.base_path),
            "Base should remain empty after restart"
        );
        assert!(
            !has_rgb_files(&config.delta_path),
            "Delta should remain empty after restart"
        );

        // Add minimal data
        let minimal_file = config.base_path.join("minimal.dat");
        std::fs::write(&minimal_file, "minimal").expect("Should create minimal file");
        ...
        let content =
            std::fs::read_to_string(&minimal_file).expect("Should read minimal file after restart");
        assert_eq!(
            content, "minimal",
            "Minimal file content should be preserved"
        );
    }
